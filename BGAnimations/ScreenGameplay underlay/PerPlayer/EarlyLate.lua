...
-- exit if disabled
if not SL["P"..pn].ActiveModifiers.EarlyLateErrorBar then return end

-- local style = SL["P"..pn].ActiveModifiers.EarlyLate
-- local color = SL["P"..pn].ActiveModifiers.EarlyLateColor

local useitg = SL.Global.GameMode == "ITG" and true or false
local env = SL.Global.GameMode
...
local windowstouse = (env == "ITG") and 2 or 3 -- decided that we don't care to show outside the first few windows
local function getWindow(n)
    ...
    window = window + prefs["TimingWindowAdd"]
